...
def render(state: AppState) -> None:
    ...
    with st.spinner("Computing metrics…"):
        ...
        clust = (
            compute_cluster_metrics(ds.cluster_labels) if ds.cluster_labels is not None else None
        )
    ...
    if clust:
        ...
        c6.metric(
            "Largest Cluster",
            f"{clust.largest_cluster_fraction:.1%}",
    ...
    with col1:
        st.plotly_chart(
            build_geometry_metrics_chart(geo), use_container_width=True, key="metrics_geo_chart"
        )
    with col2:
        st.plotly_chart(
            build_variance_per_dim_chart(geo),
            use_container_width=True,
            key="metrics_variance_chart",
        )
